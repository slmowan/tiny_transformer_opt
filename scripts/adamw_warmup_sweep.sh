...
# Run warmup sweeps for the AdamW optimizer on Tiny Shakespeare.
# Tests warmup_iters values 0, 100, 250, 500 with a fixed learning rate (6e-4)
...
RESULTS_ROOT=${RESULTS_ROOT:-out/adamw_warmup_sweep}
RESULTS_FILE=${RESULTS_FILE:-"${RESULTS_ROOT}/adamw_warmup_results.csv"}
...
for warmup in "${WARMUPS[@]}"; do
  ...
  echo "[adamw] warmup_iters=${warmup} -> ${run_dir}"
...
if not os.path.exists(ckpt_path):
    ...
    print(f"[adamw] warmup={warmup} best_val_loss={val_loss}")
